...
int binarySearch(int sortedArray[], int requestedValue, int leftIndex,
                 int rightIndex) {
  ...
  if (rightIndex >= 1) {
    ...
    if (requestedValue < sortedArray[middleIndex]) {
      return binarySearch(sortedArray, requestedValue, leftIndex,
                          middleIndex - 1);
    }
    return binarySearch(sortedArray, requestedValue, middleIndex + 1,
                        rightIndex);
  }
...
int linearSearch(const int array[], int requestedValue, int sizeOfArray) {
...
int main() {
  ...
  for (int i = 0;
       i < (sizeof vorlesungsInformationen) / (sizeof(VorlesungsInformation));
       ++i) {
    ...
    printf("Vorlesung: %s\nDatum: %sDozent: %s\nRaum: %s\n\n",
           currentInformation->VorlesungsBezeichnung, currentTime,
           currentInformation->DozentenNamen,
           currentInformation->RaumKennzeichnung);
  }
}
